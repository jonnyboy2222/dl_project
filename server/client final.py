...
class UdpSender():
    ...
    def send_frame(self):
        ...
        self.cap = cv2.VideoCapture("/home/kth/robot_dev/pyqt/sample.mp4")
...
class VideoUpdateThread(QThread):
    ...
    def update_msg(self, msg, cls_id, obj_time, obj_id, position=None, lane_mask=None):
        ...
        if cls_id == 3 and position is not None and position < 15:
        ...
        # ✅ 5. 횡단보도 + 사람
        if msg[3] == 1 and cls_id == 3 and position is not None:
            if position < 20:
                state["state_msg"] = "정지"
        ...
        if cls_id == 7 and position is not None and position < 15:
            state["alert"] = "차량이 근처에 있습니다. 주의하세요."
            # 차량 관련 action_type_id가 필요하면 추가 가능

        # ✅ 7. 어린이 보호구역
        if cls_id == 1 and position is not None and position < 20:
        ...
        # ✅ 8. 초록불
        if cls_id == 10 and position is not None and position < 20:
            state["state_msg"] = "주행 가능"

        # ✅ 9. 황색불
        if cls_id == 11 and position is not None and position < 20:
            state["alert"] = "황색 신호입니다. 감속 주의."

        # ✅ 10. 속도 제한 30
        if cls_id == 4 and position is not None and position < 20:
        ...
        # ✅ 11. 속도 제한 50
        if cls_id == 5 and position is not None and position < 20:
        ...
        # ✅ 12. 공사장
        if cls_id == 2 and position is not None and position < 20:
        ...
        # ✅ 13. 정지 표지판
        if cls_id == 6 and position is not None and position < 20:
